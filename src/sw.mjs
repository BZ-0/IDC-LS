...
const attemp = (req, event)=>{
    const sendResponse = async (response)=>{
        ...
        try { await cache.add(resp); } catch(e) { console.warn(e); };
        return resp;
    }
    ...
    if (relPath.startsWith("/opfs")) {
        return (async()=> {
            ...
            const result = sendResponse(new Response(await filex)).catch((_) => null);
            event?.waitUntil?.(result);
            return result;
        })();
    } else {
        const fc = new Promise((resolve, reject)=>setTimeout(()=>reject(null), NETWORK_TIMEOUT_MS)).catch(_WARN_);;
        ...
        }).then(sendResponse).catch((_) => null);
...
const fit = async (req, event)=>{
    ...
    //
    return attemp(req, event).then((r)=>r||cached).catch((e)=>{ console.warn(e); return cached});
}

//
const putCacheAll = (list)=>{
    return Promise.allSettled(list.map(async (it)=>{
        const cache = await caches.open(RUNTIME);
        try { await cache.add(it); } catch(e) {};
    })).catch(_WARN_);
}
